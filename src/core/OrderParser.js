export class OrderParser {
    static parse (lines) {
        ...
        for (let i = indexViaIFood; i < indexItems; i++) {
            if (lines[i]?.startsWith("R.") || lines[i]?.startsWith("Av.") || lines[i]?.startsWith("Rua") || lines[i]?.startsWith("Avenida")) break;
        ...
        for (let i = 0; i < indexItems; i++) {
            if (lines[i]?.startsWith("R.") || lines[i]?.startsWith("Av.") || lines[i]?.startsWith("Rua") || lines[i]?.startsWith("Avenida")) startedAddress = true;
        ...
        for (let i = indexItems + 1; i < endItemsIndex; i++) {
            let item = lines[i];
            const found = highlightKeys.find(key => item.toLowerCase().includes(key.toLowerCase()));
            if (found) {
        ...
        order.payment.collectOnDelivery = paymentText.includes("O entregador deve cobrar este valor");
